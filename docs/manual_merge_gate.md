...
The scheduled weekly workflow does not bypass or push to protected `main`. After this PR is merged, its first successful run bootstraps `publication` from `main`; later runs start from the exact fetched `publication` SHA and incorporate current `main`. Preflight, commit, and repository validation share one canonical publication inventory contract. Commit staging is the exact validated file set—not a directory-prefix allowlist—and the resulting commit tree must contain no extra output file. Before a normal push, the workflow rejects any change, deletion, or rename of a judgment record that existed at the fetched `publication` SHA, then requires that remote SHA to remain unchanged and to be an ancestor of local HEAD; merge conflicts or a concurrently advanced remote stop publication. The job has only `contents: write` and never receives `pull-requests: write`. After pushing, the same workflow re-fetches the remote SHA and revalidates the exact inventory, current pointer, manifest, generation chain, schemas, immutable judgments, and the deterministic size-bounded consumer projection regenerated from authoritative current.
