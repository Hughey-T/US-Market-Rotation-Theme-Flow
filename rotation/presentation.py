...
def build_user_view(*, regime: dict, style_factor: dict, sectors: dict, industries: dict, themes: dict, dynamic: dict, buckets: dict, companies: list[dict], history_weeks: int) -> dict:
    ...
    phase4 = _section(
        f"個別企業を調べる対象は{_names(buckets['research_now'])}です。回復条件を監視する対象は{_names(buckets['watch_recovery'])}です。長期材料はあるが現在の株価は弱い対象は{_names(buckets['long_term_context_price_weak'])}です。現在は避ける対象は{_names(buckets['avoid_now'])}です。",
        "条件を満たす対象が少ない週は、件数を埋めず現金余力と調査時間を温存します。",
        ["政策や長期材料が良くても、現在の株価が弱い対象は自動的に昇格しません。"],
        ["回復条件を監視する対象が50日移動平均線と市場平均比を回復するか"],
    )
    company_text = " ".join(
        f"{item['ticker']}（{item['theme_label']}）は{'代表企業' if item['selection_role'] == 'representative' else '広がり確認用企業'}です。{item['why']} 最重要確認事項は「{item['key_check']}」。最大の反対材料は「{item['counter_evidence']}」。"
    ...
    phase6 = _section(
        f"今週は{REGIME_LABELS.get(primary, REGIME_LABELS['unclassifiable'])}。個別企業を調べる対象は{_names(buckets['research_now'])}、回復条件を監視する対象は{_names(buckets['watch_recovery'])}、長期材料はあるが現在の株価は弱い対象は{_names(buckets['long_term_context_price_weak'])}、現在は避ける対象は{_names(buckets['avoid_now'])}です。",
