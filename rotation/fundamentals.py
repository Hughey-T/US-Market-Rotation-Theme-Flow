...
def load_fundamental_bundle(path: Path, data_date: str) -> dict:
    raw = path.read_bytes()
    ...
    return {"adapter_version": "1.0", "as_of": data_date, "source": path.as_posix(),
            "source_sha256": hashlib.sha256(raw).hexdigest(), "themes": output}


def load_point_in_time_fundamentals(path: Path, data_date: str) -> dict:
    return load_fundamental_bundle(path, data_date)["themes"]
